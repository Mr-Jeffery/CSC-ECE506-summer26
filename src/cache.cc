...
void Cache::receivePrRd(addr_t addr) {
    ...
    // A read hit on a line freshened by a peer's BusUpd consumes that update, making it useful
    else if (line->updated) {
        statistics[UsefulBusUpdate]++;
        line->updated = false;
    }
...
void Cache::receivePrWr(addr_t addr) {
    ...
    if (line && line->state) {
        // A local write supersedes any pending peer update, so it is not consumed by a read
        line->updated = false;
...
void Cache::receiveBusMsg(bus_msg_e bus_msg, addr_t addr) {
    ...
    case BusUpdate:
        ...
        // Mark the freshened copy as holding an as-yet-unconsumed update (see UsefulBusUpdate)
        line->updated = true;
#ifdef WRITE_TIMESTAMP
