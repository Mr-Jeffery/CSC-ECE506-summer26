...
/// @brief The Dragon (write-update) coherence protocol
/// @note States used: E (exclusive), M (modified), Sc (shared clean), Sm (shared modified).
///       Dragon has no invalid state for resident lines and never issues BusRdX/BusUpgr.
...
public:
    ...
    /// @brief Receive a BusUpdt (write-update) message
    /// @param line The cache line accessed
    /// @return True if the line was flushed to the bus
